...
export default function CenterLogo({ className = "" }: { className?: string }) {
  ...
  const hex = `hex_${uid}`;
  const bulb = `bulb_${uid}`;
  const book = `book_${uid}`;
  ...
  return (
    <svg viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg" className={className}>
      <defs>
        ...
        <linearGradient id={bulb} x1="0" y1="1" x2="0" y2="0">
          <stop offset="0%" stopColor="#d97706" />
          <stop offset="100%" stopColor="#fde68a" />
        </linearGradient>
        <linearGradient id={book} x1="0" y1="0" x2="1" y2="0">
      ...
      {/* Open book - bottom */}
      <path d="M13 43 Q22 38 31 39 L31 52 Q22 51 13 54 Z" fill={`url(#${book})`} opacity="0.92" />
      ...
      {/* Lightbulb body */}
      <circle cx="32" cy="24" r="11" fill={`url(#${bulb})`} filter={`url(#${glow})`} />
